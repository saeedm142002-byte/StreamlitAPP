...
def build_new_collectors(main_df, neglect_df,
    ...
    # الهدف الاجمالي لكل المحصلين الجدد مع بعض (عشان يتوزعوا بالتساوي بينهم)
    n_new_collectors = len(new_collectors)
    total_target_debt = target_debt * n_new_collectors
    total_target_accounts = target_accounts * n_new_collectors
    total_target_ids = target_ids * n_new_collectors
    ...
    # 3) توزيع على المحصلين الجداد بالتساوي بينهم، لحد ما الاجمالي يوصل للهدف
    #    (كل المحصلين الجدد فاضلين "مفتوحين" طول الوقت، والمجموعة الجديدة بتروح
    #     دايمًا لأقل واحد فيهم مديونية، عشان التوازن يفضل محافظ عليه بينهم)
    bins = {c: {"debt": 0.0, "accounts": 0, "ids": 0, "rows": []} for c in new_collectors}
 
    for grp in groups:
        cur_debt = sum(b["debt"] for b in bins.values())
        cur_accounts = sum(b["accounts"] for b in bins.values())
        cur_ids = sum(b["ids"] for b in bins.values())
        if (cur_debt >= total_target_debt and
                cur_accounts >= total_target_accounts and
                cur_ids >= total_target_ids):
            break
 
        target_c = min(bins, key=lambda c: bins[c]["debt"])  # اقل واحد مديونية ياخد
...
def balance_portfolio(df, id_col, collector_col, supervisor_col, account_col, debt_col,
    ...
    existing_collectors = sorted(work[collector_col].dropna().astype(str).unique().tolist())
    all_collectors = existing_collectors + new_collectors
 
    def safe_mode(s):
        m = s.dropna().mode()
        return m.iat[0] if len(m) else "غير محدد"
 
    sup_map = {c: safe_mode(work.loc[work[collector_col] == c, supervisor_col])
